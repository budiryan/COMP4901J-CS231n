...
class TwoLayerNetLeaky(object):
  """
  BONUS PART FOR ASSIGNMENT 1: Implementation of 2 Layer NN with Leaky ReLU
                               activation function

  In other words, the network has the following architecture:

  input - fully connected layer - Leaky ReLU - fully connected layer - softmax
  """

  def __init__(self, input_size, hidden_size, output_size, std=1e-4, leaky_alpha=0.01):
    """
    Initialize the model. Weights are initialized to small random values and
    biases are initialized to zero. Weights and biases are stored in the
    variable self.params, which is a dictionary with the following keys:
